...
public:
  ...
  [[maybe_unused]] OperationList& AddOperation(Operation operation, std::function<double(double, double)> action){
    if(operations_.contains(operation)){
      throw std::runtime_error("Operation - " + operation.token + "already added to operation list!");
    }
    ...
    operations_[operation] = std::make_shared<BinaryOperationAction>(BinaryOperationAction(std::move(action)));
    operations_tokens_[operation.token] = operation;
    return *this;
  }

  [[maybe_unused]] OperationList& AddOperation(Operation operation, std::function<double(double)> action){
    if(operations_.contains(operation)){
      throw std::runtime_error("Operation - " + operation.token + "already added to operation list!");
    }
    lexema_maexpression_variable_len = operation.token.size() > lexema_maexpression_variable_len ? operation.token.size() : lexema_maexpression_variable_len;
    operations_[operation] = std::make_shared<UnaryOperationAction>(UnaryOperationAction(std::move(action)));
    operations_tokens_[operation.token] = operation;
    return *this;
  }
...
public:
  ...
  std::shared_ptr<Lexema> GetLexema(const std::string& str, std::string::iterator& it) {
    ...
    if(current_lexem.get() == nullptr){
      ...
      throw std::runtime_error( "Not found a lexem at " + std::to_string(dis) + " position");
    }
...
public:
  ...
  double CaluclateExpression(std::string expression, double expression_variable){
